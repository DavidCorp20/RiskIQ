...
export default function AnalysisLibrary({datasetId='',onContinue}){
  ...
  const selectedReady=Boolean(current?.readiness?.ready===true)
      ...
      <div className="analysis-library-grid">
        ...
        {current&&<div className="analysis-model-detail">
          ...
          <div className="analysis-model-actions">
            <span>{selectedReady?'MODELO LISTO':'MODELO NO DISPONIBLE'}</span>
            <button type="button" onClick={onContinue} disabled={!selectedReady||!onContinue}>{selectedReady?'Continuar a Decisions →':'Requiere evidencia adicional'}</button>
          </div>
        </div>}
      </div>
