...
export default function DatasetWorkspace({activeDataset,onSelect,onResult}){
  ...
  async function select(id){
    setLoading(true);setError('')
    try{const r=await getDataset(id);onSelect(r)}catch(e){setError(e.message)}finally{setLoading(false)}
  }
  async function run(){
    if(!activeDataset?.dataset_id)return
    setRunning(true);setError('')
    try{const r=await runDataset(activeDataset.dataset_id);onResult(r)}catch(e){setError(e.message)}finally{setRunning(false)}
  }
