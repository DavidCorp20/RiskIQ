...
export default function DecisionCenter({decisionEngine,result,priorities=[],datasetId}){
 ...
 const path=selected?[
   {stage:'FACTS',label:'Facts',evidence:facts},
   {stage:'RULE',label:'Rule',evidence:selectedRuleId||selected.trigger||selected.reason},
   {stage:'DECISION',label:'Decision',evidence:selected.title||selected.recommended_action},
   {stage:'IMPACT',label:'Impact',evidence:impact??'—'},
   {stage:'HUMAN_REVIEW',label:'Human review',evidence:selected.requires_human_review===false?'Not required':'Required'},
 ...
 return <section className="dc-shell">
  ...
  {!cards.length?<div className="dc-empty"><strong>No hay decisiones para escalar.</strong><span>Ejecuta el análisis o ajusta la política para generar casos revisables.</span></div>:<div className="dc-layout">
   ...
   <div className="dc-detail"><div className="dc-decision-hero"><div><span>DECISIÓN RESULTANTE</span><strong>{decision}</strong><small>{selected?.reason||selected?.rationale||'Decisión derivada de evidencia calculada.'}</small></div>{score!=null&&<div className="dc-score"><span>RISK SCORE</span><strong>{fmt(score)}</strong></div>}</div>
    ...
    <div className="dc-two"><Panel title="INPUT & FACTS"><div className="dc-facts">{Object.entries(facts).slice(0,16).map(([k,v])=><div key={k}><span>{k}</span><b>{typeof v==='object'?JSON.stringify(v):fmt(v)}</b></div>)}</div>{!Object.keys(facts).length&&<div className="dc-muted">No se recibió un snapshot de facts para este caso.</div>}</Panel><Panel title="CASE EVIDENCE"><div className="dc-facts">{Array.isArray(selectedEvidence)?selectedEvidence.map((item,i)=><div key={i}><span>EVIDENCIA {i+1}</span><b>{label(item)}</b></div>):Object.entries(selectedEvidence||{}).map(([k,v])=><div key={k}><span>{label(k)}</span><b>{typeof v==='object'?JSON.stringify(v):fmt(v)}</b></div>)}</div>{!Object.keys(selectedEvidence||{}).length&&<div className="dc-muted">No hay evidencia específica asociada a este caso.</div>}</Panel></div>
