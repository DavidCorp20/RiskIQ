...
export default function DecisionCenter({decisionEngine,result,priorities=[],datasetId}){
 ...
 const cardEvidence=selected?.evidence
 const facts=evidence.facts||{}
 const score=evidence.scorecard?.score
 const triggeredRules=Array.isArray(evidence.triggered_rules)?evidence.triggered_rules:engine.triggered_rules||[]
 const reasonCodes=Array.isArray(evidence.reason_codes)?evidence.reason_codes:engine.reason_codes||[]
 const decision=evidence.decision||selected?.recommended_action||selected?.recommendation||'REVIEW'
 ...
 return <section className="dc-shell">
  ...
  {!cards.length?<div className="dc-empty"><strong>No hay decisiones para escalar.</strong><span>Ejecuta el análisis o ajusta la política para generar casos revisables.</span></div>:<div className="dc-layout">
   ...
   <div className="dc-detail"><div className="dc-decision-hero"><div><span>DECISIÓN RESULTANTE</span><strong>{decision}</strong><small>{selected?.rationale||'Decisión derivada de evidencia calculada.'}</small></div>{score!=null&&<div className="dc-score"><span>RISK SCORE</span><strong>{fmt(score)}</strong></div>}</div>
    ...
    <div className="dc-two"><Panel title="INPUT & FACTS"><div className="dc-facts">{Object.entries(facts).slice(0,16).map(([k,v])=><div key={k}><span>{k}</span><b>{typeof v==='object'?JSON.stringify(v):fmt(v)}</b></div>)}</div>{!Object.keys(facts).length&&<div className="dc-muted">No se recibió un snapshot de facts para este caso.</div>}</Panel><Panel title="POLICY MATCH"><div className="dc-match"><div><span>TRIGGERED RULES</span><strong>{triggeredRules.length}</strong></div><div><span>REASON CODES</span><strong>{reasonCodes.length}</strong></div></div><p>{evidenceText(reasonCodes)||'La decisión no expone códigos de razón adicionales.'}</p></Panel></div>
    ...
    <details className="dc-technical"><summary>Evidence trace · técnico</summary><pre>{JSON.stringify({facts,scorecard:evidence.scorecard,triggered_rules:triggeredRules,reason_codes:reasonCodes,policy,ledger_count:audit.length},null,2)}</pre></details>
   </div>
  </div>}
 </section>
}
