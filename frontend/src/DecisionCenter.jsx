...
export default function DecisionCenter({decisionEngine,result,priorities=[],datasetId}){
 ...
 return <section className="dc-shell">
  ...
  {!cards.length?<div className="dc-empty"><strong>No hay decisiones para escalar.</strong><span>Ejecuta el análisis o ajusta la política para generar casos revisables.</span></div>:<div className="dc-layout">
   <div className="dc-list"><div className="dc-list-head"><span>QUEUE</span><b>{cards.length} casos</b></div>{cards.map((card,i)=><button key={card.id||card.decision_id||i} className={`dc-case ${selected===card?'selected':''}`} onClick={()=>setSelected(card)}><div><span className={`dc-severity ${card.severity||card.priority||'watch'}`}>{tone[card.severity]||String(card.severity||card.priority||'WATCH').toUpperCase()}</span><strong>{card.title||card.name||card.decision_code||`Decisión ${i+1}`}</strong><small>{card.rationale||card.recommended_action||card.recommendation||'Revisión requerida'}</small></div><b>›</b></button>)}</div>
