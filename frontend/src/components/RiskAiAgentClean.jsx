...
import { Activity, Bot, Database, Send, ShieldCheck, Sparkles, TrendingDown, X } from 'lucide-react'
...
function buildLocalAnswer(question, context) {
  ...
  if (/par\s*30|30\+|mora 30/.test(query)) return `PAR30 actual: ${formatPercentage(context.par30)}, equivalente a ${formatMoney(context.exposure * context.par30)} de exposición aproximada en 30+ sobre ${formatMoney(context.exposure)} de exposición total.`
  if (/par\s*60|60\+|mora 60/.test(query)) return `PAR60 actual: ${formatPercentage(context.par60)}, equivalente a ${formatMoney(context.exposure * context.par60)} de exposición aproximada en 60+ sobre ${formatMoney(context.exposure)} de exposición total.`
  if (/par\s*90|90\+|mora 90/.test(query)) return `PAR90 actual: ${formatPercentage(context.par90)}, equivalente a ${formatMoney(context.exposure * context.par90)} de exposición aproximada en 90+ sobre ${formatMoney(context.exposure)} de exposición total.`
  if (/expos|saldo|balance|outstanding/.test(query)) return `La exposición total calculada es ${formatMoney(context.exposure)}. PAR30 es ${formatPercentage(context.par30)}, PAR60 es ${formatPercentage(context.par60)} y PAR90 es ${formatPercentage(context.par90)}.`
      ...
      <div className="flex min-h-[calc(100vh-145px)] flex-col lg:flex-row">
        <section className="flex min-w-0 flex-1 flex-col">
          <header className="flex flex-col gap-4 border-b border-slate-200 px-5 py-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-900 text-white"><Bot size={20} strokeWidth={1.5} /></div>
            ...
            <div className="flex items-center gap-2">
              ...
              {onClose ? (
                <button type="button" onClick={onClose} aria-label="Cerrar Risk AI Agent" className="flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-900"><X size={15} strokeWidth={1.5} /></button>
              ) : null}
            </div>
          </header>

          <div className="grid grid-cols-3 divide-x divide-slate-200 border-b border-slate-200 bg-white">
            <div className="px-4 py-3"><div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">PAR30</div><div className="mt-1 text-lg font-semibold tabular-nums text-slate-900">{formatPercentage(context.par30)}</div></div>
            <div className="px-4 py-3"><div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">PAR60</div><div className="mt-1 text-lg font-semibold tabular-nums text-slate-900">{formatPercentage(context.par60)}</div></div>
            <div className="px-4 py-3"><div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">PAR90</div><div className="mt-1 text-lg font-semibold tabular-nums text-slate-900">{formatPercentage(context.par90)}</div></div>
          </div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50/50 p-4 sm:p-6">
            {messages.length === 0 ? (
              <div className="mx-auto flex min-h-[360px] max-w-2xl flex-col items-center justify-center text-center">
                <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-700 shadow-sm"><Sparkles size={24} strokeWidth={1.5} /></div>
                ...
                <div className="mt-5 grid w-full max-w-xl gap-2 sm:grid-cols-2">
                  {quickPrompts.map((prompt) => <button key={prompt} type="button" onClick={() => handleSend(prompt)} className="rounded-lg border border-slate-200 bg-white px-3 py-3 text-left text-sm text-slate-700 transition hover:border-slate-300 hover:bg-slate-50">{prompt}</button>)}
                </div>
              </div>
            ) : null}
            ...
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[88%] rounded-xl border px-4 py-3 text-sm leading-6 shadow-sm ${message.role === 'user' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700'}`}>
                  ...
                  {message.decision?.suggested_action ? <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800"><strong>Acción sugerida:</strong> {message.decision.suggested_action}</div> : null}
                  {message.source === 'local-fallback' ? <div className="mt-2 text-[10px] font-semibold uppercase tracking-wider text-amber-600">Evidence fallback · backend no disponible</div> : null}
                  {message.provider && message.role !== 'user' ? <div className="mt-2 text-[10px] uppercase tracking-wider text-slate-400">{message.provider}</div> : null}
                </div>
              </div>
            ))}

            {isAnalyzing ? (
              <div className="flex justify-start"><div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-500 shadow-sm"><Activity size={15} className="animate-pulse" strokeWidth={1.5} />Analizando evidencia…</div></div>
            ) : null}
          </div>
          ...
          <footer className="border-t border-slate-200 bg-white p-4 sm:p-5">
            <div className="flex items-end gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2 focus-within:border-slate-400">
              <textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={handleKeyDown} rows={2} placeholder="Pregunta por riesgo, mora, concentración o deterioro…" className="min-h-[48px] flex-1 resize-none border-0 bg-transparent px-2 py-2 text-sm text-slate-800 outline-none placeholder:text-slate-400" />
              <button type="button" aria-label="Enviar consulta" disabled={!input.trim() || isAnalyzing} onClick={() => handleSend()} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-900 text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40"><Send size={16} strokeWidth={1.5} /></button>
            </div>
        ...
        <aside className="w-full shrink-0 border-t border-slate-200 bg-white lg:w-[330px] lg:border-l lg:border-t-0">
          ...
          <div className="space-y-4 p-5">
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4"><div className="text-xs text-slate-500">Dataset</div><div className="mt-1 truncate text-sm font-medium text-slate-800">{context.datasetId || 'Sin dataset'}</div></div>
            <div className="rounded-lg border border-slate-200 bg-white p-4"><div className="text-xs text-slate-500">Exposición total</div><div className="mt-1 text-xl font-semibold tabular-nums text-slate-900">{formatMoney(context.exposure)}</div></div>
            <div className="rounded-lg border border-slate-200 bg-white p-4">
              <div className="mb-2 flex items-center gap-2"><TrendingDown size={14} className="text-slate-500" strokeWidth={1.5} /><span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Prioridades</span></div>
              {context.priorities.length > 0 ? context.priorities.slice(0, 4).map((item, index) => (
                <div key={`${item.rank || index}-${item.title || 'priority'}`} className="border-t border-slate-100 py-2 first:border-t-0"><div className="text-xs font-medium text-slate-700">#{item.rank || index + 1} {item.title || 'Prioridad'}</div><div className="mt-0.5 line-clamp-2 text-[11px] leading-4 text-slate-500">{item.why || 'Evidencia calculada disponible'}</div></div>
              )) : <div className="text-xs text-slate-500">No hay prioridades calculadas.</div>}
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4"><div className="flex items-center gap-2 text-xs font-medium text-slate-600"><ShieldCheck size={14} strokeWidth={1.5} />Grounded analysis</div><p className="mt-1 text-[11px] leading-4 text-slate-500">La IA interpreta evidencia calculada y mantiene revisión humana para decisiones.</p></div>
          </div>
        </aside>
      </div>
    </div>
