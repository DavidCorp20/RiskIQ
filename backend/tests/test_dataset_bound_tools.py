...
async def test_copilot_requires_real_dataset(monkeypatch):
    fake = FakePersistence()
    fake.portfolio_records = FakeCollection([{"dataset_id": "ds-1", "loan_id": "L1", "outstanding_principal": 1000, "dpd": 0, "snapshot_date": "2026-09-10"}])
    fake.loans = FakeCollection([])
    fake.installments = FakeCollection([])
    monkeypatch.setattr(ai_routes, "persistence", fake)
