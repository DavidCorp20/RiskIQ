...
class DecisionPipelineService:
    ...
    def build(
        ...
        metrics = {
            ...
            "exposure": current.get("outstanding_balance", 0),
            ...
            for alert in history["alerts"]:
                ...
                if code == "PAR30_DETERIORATION":
                    decisions["recommendations"].append({
                        "code": "INVESTIGATE_PAR30_TREND",
                        "title": "Investigar deterioro reciente de PAR30",
                        "priority": "high",
                        "rationale": "PAR30 aumentó frente al snapshot anterior; revisar vintage, segmentos y efectividad de cobranza.",
                        "evidence": {
                            "current_par30": history["changes"]["par30"]["current"],
                            "previous_par30": history["changes"]["par30"]["previous"],
                            "delta": history["changes"]["par30"]["delta"],
                        },
                        "mode": "suggested",
                    })
                elif code == "PAR90_DETERIORATION":
                    decisions["recommendations"].append({
                        "code": "ESCALATE_PAR90_TREND",
                        "title": "Escalar deterioro reciente de PAR90",
                        "priority": "critical",
                        "rationale": "PAR90 aumentó frente al snapshot anterior; priorizar análisis de recuperación y exposición afectada.",
                        "evidence": {
                            "current_par90": history["changes"]["par90"]["current"],
                            "previous_par90": history["changes"]["par90"]["previous"],
                            "delta": history["changes"]["par90"]["delta"],
                        },
                        "mode": "suggested",
                    })
        ...
        if custom_rules:
            ...
            for action in custom_result["actions"]:
                ...
                decisions["recommendations"].append({
                    "code": f"CUSTOM_RULE_{rule_id.upper()}",
                    "title": self._action_title(action_type),
                    "priority": priority,
                    "rationale": "Regla institucional configurada por el usuario y activada por los hechos actuales.",
                    "evidence": {"rule_id": rule_id, "action_type": action_type, "facts": metrics},
                    "mode": action.get("mode", "suggested"),
                })
        ...
        return {
            ...
            "cards": {"count": len(cards), "items": cards},
        }

    @staticmethod
    def _action_title(action_type: str) -> str:
        return {
            "alert": "Revisar alerta de regla institucional",
            "recommend": "Evaluar recomendación de regla institucional",
            "set_risk_level": "Revisar nivel de riesgo configurado",
            "review": "Revisar decisión institucional",
            "block": "Revisar bloqueo definido por regla",
        }.get(action_type, "Revisar regla institucional")
