...
class DataQualityService:
    ...
    def assess(self, rows: list[dict[str, Any]], *, mappings: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        for index, row in enumerate(rows):
            ...
            if loan_id in (None, ""):
                add_row_issue(index, "MISSING_LOAN_ID", "critical", "loan_id", "Falta el identificador del crédito/cuenta.")
            else:
                key = str(loan_id)
                seen_ids[key] = seen_ids.get(key, 0) + 1
                if seen_ids[key] > 1:
                    duplicate_ids.add(key)
                    add_row_issue(index, "DUPLICATE_LOAN_ID", "critical", "loan_id", "El identificador del crédito aparece más de una vez.")

            if row.get("customer_id") in (None, ""):
                add_row_issue(index, "MISSING_CUSTOMER_ID", "critical", "customer_id", "Falta el identificador del cliente.")
            ...
            for field in self.NUMERIC_FIELDS:
                ...
                try:
                    ...
                    if field == "dpd" and value < 0:
                        add_row_issue(index, "NEGATIVE_DPD", "high", field, "Los días de mora no pueden ser negativos.")
                    elif field in {"outstanding_principal", "scheduled_amount", "paid_amount", "ead"} and value < 0:
                        add_row_issue(index, f"NEGATIVE_{field.upper()}", "high", field, "El valor monetario no puede ser negativo.")
                    elif field in {"pd", "lgd"} and not 0 <= value <= 1:
                        add_row_issue(index, f"INVALID_{field.upper()}_RANGE", "high", field, "El indicador debe estar entre 0 y 1.")
                except (TypeError, ValueError):
                    field_stats.setdefault(field, {"present": 0, "empty": 0, "invalid": 0})["invalid"] += 1
                    add_row_issue(index, f"INVALID_NUMBER_{field.upper()}", "high", field, "El valor no es numérico.")

            for field in self.DATE_FIELDS:
                if field in row and row[field] not in (None, "") and not self._valid_date(row[field]):
                    field_stats.setdefault(field, {"present": 0, "empty": 0, "invalid": 0})["invalid"] += 1
                    add_row_issue(index, f"INVALID_DATE_{field.upper()}", "high", field, "La fecha no tiene un formato válido.")
            ...
            if scheduled > 0 and paid > scheduled * 1.05:
                add_row_issue(index, "PAID_ABOVE_SCHEDULED", "medium", "paid_amount", "El pago supera en más de 5% al monto programado.")
        ...
        for field, stats in field_stats.items():
            ...
            if missing_rate >= 0.1 or invalid_rate > 0:
                column_issues.append({
                    ...
                    "severity": "critical" if field in {"loan_id", "customer_id"} and missing_rate > 0 else "warning",
                })

        issues_list = [{"code": code, "severity": severity, "count": count} for (code, severity), count in sorted(issues.items())]
        quality = self._analysis_impacts(rows, columns)
        return {
            "quality_score": score,
            "status": status,
            "row_count": len(rows),
            ...
            "analysis_impacts": quality,
            "checks": {
                "completeness": round(completeness, 4),
                "validity": round(validity, 4),
                "unique_loan_ids": len([value for value in seen_ids if seen_ids[value] == 1]),
                "duplicate_loan_ids": len(duplicate_ids),
            },
            "mapping": {"provided": mappings is not None, "mapped_fields": len(mappings or [])},
            "policy": "Errors block affected analysis; warnings remain processable and are surfaced with their analytical impact.",
        }

    @staticmethod
    def _analysis_impacts(rows: list[dict[str, Any]], columns: list[str]) -> list[dict[str, Any]]:
        def has(field: str) -> bool:
            return field in columns and any(row.get(field) not in (None, "") for row in rows)
        ...
        for model, requires, label in [
            ...
            impacts.append({
                ...
                "severity": "none" if not missing else ("warning" if model in {"Vintage Analysis", "Migration / Roll Rate"} else "blocking"),
            })
        return impacts
