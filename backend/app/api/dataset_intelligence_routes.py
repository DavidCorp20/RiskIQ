...
def _require_dataset(dataset_id:str)->dict[str,Any]:
    ...
    if not rows: raise HTTPException(status_code=404,detail="Dataset not found")
    return rows[0]
...
def _advanced_analytics(risk:dict[str,Any],vintage_analysis:dict[str,Any],current:dict[str,Any],previous:dict[str,Any]|None)->dict[str,Any]:
    par=risk.get("par") or {};concentration=risk.get("concentration") or {};segments=concentration.get("segments") or [];products=concentration.get("products") or [];stress=[];base_exposure=float(risk.get("exposure") or 0);base_par30_balance=float((par.get("par30") or {}).get("balance") or 0)
    for shock in (.10,.20,.30):
        stressed=base_par30_balance*(1+shock);stress.append({"shock_pct":shock,"additional_at_risk":round(base_par30_balance*shock,2),"stressed_par30":round(stressed/base_exposure,4) if base_exposure else 0})
    if previous:
        prev_par30=float(previous.get("par30") or 0);current_par30=float((par.get("par30") or {}).get("ratio") or 0);migration={"available":True,"previous_par30":prev_par30,"current_par30":current_par30,"delta_par30":round(current_par30-prev_par30,4),"direction":"deteriorating" if current_par30>prev_par30 else "improving" if current_par30<prev_par30 else "stable"}
    else: migration={"available":False,"reason":"Se requiere al menos un corte histórico anterior."}
    return {"risk_profile":[{"metric":key.upper(),"ratio":float((value or {}).get("ratio") or 0),"balance":float((value or {}).get("balance") or 0),"loans":int((value or {}).get("loans") or 0)} for key,value in par.items()],"concentration":{"top_segments":segments[:8],"top_products":products[:8],"segment_concentration_ratio":round(sum(float(x.get("share_of_exposure") or 0) for x in segments[:3]),4)},"migration":migration,"cohorts":risk.get("vintage") or vintage_analysis or [],"stress":stress,"methodology":{"deterministic":True,"stress_is_hypothetical":True,"migration_requires_history":True,"cohorts_use_origination_date_when_available":True,"causality_inferred":False}}
...
def _latest_source_date(records:list[dict[str,Any]])->str|None:
    dates=[]
    for row in records:
        for key in ("snapshot_date","snapshot_month","as_of_date"):
            if row.get(key) not in (None,""):
                dates.append(str(row[key])[:10]);break
...
def run_dataset_workspace(dataset_id:str,payload:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    return {"status":result.get("status","healthy"),"contract_version":"dataset-intelligence-v6","dataset":metadata,"dataset_id":dataset_id,"snapshot":{"id":snapshot_id,**current},"previous_snapshot":previous,"analysis":analysis,"risk_analytics":{"deterministic":risk,"npl":npl_analysis},"advanced_analytics":advanced,"risk_intelligence":intelligence_result,"decision_engine":decisions,"decision_evidence_ledger":{"count":len(ledger_entries),"entries":ledger_entries},"vintage":vintage_analysis,"workspace":result,"governance":{"real_dataset":True,"analytics_are_deterministic":True,"decision_engine_is_deterministic":True,"customer_actions_executed":False,"human_review_required":True,"trend_available":previous is not None,"roll_rate_requires_historical_snapshots":True,"causality_inferred":False,"npl_is_regulatory_definition":False,"risk_intelligence_version":"risk-intelligence-v1","decision_ledger_persisted":True,"as_of":as_of}}
