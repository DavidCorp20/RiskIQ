...
def _advanced_analytics(risk: dict[str, Any], vintage_analysis: dict[str, Any], current: dict[str, Any], previous: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for shock in (0.10, 0.20, 0.30):
        ...
        stress.append({"shock_pct": shock, "additional_at_risk": round(base_par30_balance * shock, 2), "stressed_par30": round(stressed_balance / base_exposure, 4) if base_exposure else 0})
    if previous:
        prev_par30 = float(previous.get("par30") or 0)
        current_par30 = float((par.get("par30") or {}).get("ratio") or 0)
        migration = {"available": True, "previous_par30": prev_par30, "current_par30": current_par30, "delta_par30": round(current_par30 - prev_par30, 4), "direction": "deteriorating" if current_par30 > prev_par30 else "improving" if current_par30 < prev_par30 else "stable"}
    else:
        migration = {"available": False, "reason": "Se requiere al menos un corte histórico anterior."}
    return {
        "risk_profile": [{"metric": key.upper(), "ratio": float((value or {}).get("ratio") or 0), "balance": float((value or {}).get("balance") or 0), "loans": int((value or {}).get("loans") or 0)} for key, value in par.items()],
        "concentration": {"top_segments": segments[:8], "top_products": products[:8], "segment_concentration_ratio": round(sum(float(x.get("share_of_exposure") or 0) for x in segments[:3]), 4)},
        "migration": migration,
