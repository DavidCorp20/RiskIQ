...
from app.risk_events.repository import FreshserviceRiskActionAdapter,RiskActionRepository,RiskEventRepository
from app.risk_events.schemas import RiskActionStatusUpdate,RiskEventDetectionRequest,RiskEventStatusUpdate
router=APIRouter(prefix="/v1/risk-events",tags=["risk-events"]);event_engine=RiskEventEngine();action_engine=ActionEngine();events=RiskEventRepository();actions=RiskActionRepository();freshservice=FreshserviceRiskActionAdapter()
@router.post("/detect")
async def detect(request:RiskEventDetectionRequest,background_tasks:BackgroundTasks):
    ...
    for event in event_engine.detect(request.dataset_id,request.ews_results,request.snapshot_id):
        ...
        if request.auto_orchestrate:
            action=actions.create_if_absent(action_engine.create_action(event));action_rows.append(action);background_tasks.add_task(freshservice.sync_action,action,row)
    return {"count":len(event_rows),"events":event_rows,"actions":action_rows,"idempotent":True}
