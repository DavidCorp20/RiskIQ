...
service=RuleBuilder(); engine=DecisionEngine(); formula_engine=FormulaEngine(); rules_repo=DecisionRuleRepository()
@router.get("/rules")
...
def compile_rule(payload:dict)->dict:
    ...
    if result["valid"]: result["compiled_rule"]=dict(result["compiled_rule"],builder_version="4.0",contract="riskiq-decision-engine")
    return result
@router.post("/save")
...
def run_pipeline(payload:dict)->dict:
    ...
    return {"stages":[x for x in ["normalize" if normalization else None,"facts","formulas" if payload.get("formulas") else None,"logic","decision"] if x],"result":result,"execution_mode":"test_only","customer_actions_executed":False}
